...
- Status counts: `2851 supported`, `776 xfail`, `0 skip`
- Purity split: `supported_semantic=2851`, `supported_pure=2851`, `supported_impure=0`
- Direct Cypher total snapshot: `2834 / 3627` (`78.1%`)
- Direct Cypher non-validation debt: `24`
...
Current split is `21 success_wrong_rows` and
`3 unexpected_success_expected_error`. The focused details suggest the debt is
not one uniform class:

| likely class | keys | note |
|---|---:|---|
| Numeric/string/display normalization | 5 | String escaping, map key order, label order, and remaining display-style mismatches. Examples: `expr-literals6-5`, `expr-literals7-18`, `expr-literals8-18`, `match3-7`. Seven integer/float and exponent-formatting cases were promoted by the Step 17 numeric-equivalence slice; four `toString(boolean)` string-keyword rendering cases and two nested numeric literal display cases were promoted by later normalization slices. |
