...
- Default local validation in this repo: `./bin/ci.sh`, `pytest tests/cypher_tck -xvs`, and `python -m tests.cypher_tck.report`.
- Keep plan files under `plans/<task>/` (already gitignored here).
