...
### Added
- **Direct-Cypher xfail contract**: Updated sibling-target drift snapshot for `match7-24` from `GFQLValidationError` to `success_matches_expected`.
...
- **Direct-Cypher row promotion (`with5-2` DISTINCT map-list aggregate)**: Added `with5-2` to the sibling-target `success_matches_expected` drift bucket after the current pygraphistry branch target returned the expected row, keeping the branch-paired TCK contract aligned for pygraphistry#1489 CI.
