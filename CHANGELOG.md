...
- **Direct-Cypher pattern predicate rebaseline**: Moved `expr-pattern1-{10,13,18}` from `success_wrong_rows` to `success_matches_expected` after pygraphistry #1451 ("Fix Cypher pattern predicate row expansion") landed on master and these Pattern1 scenarios now return the expected rows.
