...
- **Fixture parser — CREATE property references**: `graph_fixture_from_create` now resolves `<var>.<prop>` property references in `CREATE` setups (e.g. `CREATE (a:End {id: 0}), (:Begin {num: a.id})` gives the Begin node `num = 0`) instead of materializing the raw token `"a.id"` as a string. Unresolvable references are left as text. Addresses the property-reference half of tck-gfql#126; the identity/`id`-property conflation half is blocked on pygraphistry#1490 (the Cypher engine hardcodes `id` as the node-identity column) and is tracked there.
