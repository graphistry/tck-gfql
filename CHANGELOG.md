...
### Added
- **Scenario Outline placeholder substitution converter (`<predicate>`, `<rhs>`, `<operands>`)**: Added a narrow loader-side substitution pass for the audited #119 cluster using upstream openCypher TCK Examples-table values. This clears literal placeholders from all 68 cluster-1 generated scenarios, promotes 11 now-valid direct-Cypher row scenarios (`expr-comparison2-5-{1..4}`, `expr-comparison2-6-{1,2}`, `expr-quantifier7-3-{1..5}`), and reclassifies `expr-comparison2-6-{3,4}` as true `success_wrong_rows` runtime residuals instead of syntax-placeholder harness failures.
