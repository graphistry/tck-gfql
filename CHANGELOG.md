...
- **Capability/debt manifest expected-error validation**: Bumped the manifest
  ...
  validator remains backward compatible for entries without the block when no
  case-level actual error payload is present, and fails on stale structured
  error claims or actual-error drift with anchored diagnostics (tck-gfql#166).
- **Direct-Cypher `match5-8` bookkeeping closeout**: Added a focused regression guard confirming `match5-8` is promoted through the direct-Cypher row support snapshot, absent from non-validation debt, absent from `success_matches_expected`, and represented as supported direct-Cypher-only metadata in the capability/debt manifest (tck-gfql#167).
- **Unified conformance summary artifact**: Added `tests.cypher_tck.unified_conformance_summary`, a PR-comment-ready aggregation tool that consumes the #147 report JSON, #152 capability/debt manifest, and #156 snapshot-delta artifacts and writes both markdown and schema-versioned JSON without changing the input artifact schemas (tck-gfql#162).
- **Capability/debt manifest**: Added a schema-versioned scenario manifest and validator for current support/debt metadata. The validator runs in the pytest suite, reconciles against the #147 JSON report artifact contract, and fails on stale scenario keys, missing debt reasons, aggregate drift, or direct-Cypher debt-key mismatches (tck-gfql#152).
- **Conformance report JSON artifact**: `python -m tests.cypher_tck.report` now writes a stable schema-versioned JSON artifact at `build/cypher-tck-report.json` by default, with `--json-output` for custom paths. The artifact preserves existing count semantics for downstream conformance handoffs and includes source refs, scenario/GFQL/direct-Cypher counts, expected-error counts, debt keys, and optional runtime profile labels (tck-gfql#147).
- **Direct-Cypher ORDER BY scope recovery**: Rebaselined the direct-Cypher overlap snapshot after the paired pygraphistry raw-Cypher fix for stale aliases in `ORDER BY` after `WITH` projection. Restored `with-orderby1-46-{1,2,3,4,5}` to the live overlap set, raising overlap support from 1955 to 1960 while leaving promoted-only support unchanged at 949 (807 rows, 142 expected-error promotions) (tck-gfql#139).
- **Direct-Cypher literal overlap recovery**: Rebaselined the direct-Cypher overlap snapshot after the paired pygraphistry runtime fix for double-quoted string literal projections and `false`/`null` inside list literals. Restored `expr-literals6-11`, `expr-literals7-2`, and `expr-literals7-3` to the live overlap set, raising overlap support from 1952 to 1955 while leaving promoted-only support unchanged at 949 (807 rows, 142 expected-error promotions) (tck-gfql#140).
- **Direct-Cypher promotion snapshot refresh**: Fixed direct-Cypher sweep classification for `cypher-string-error` scenarios and regenerated the support snapshot to 949 promoted-only scenarios (807 row-oracle promotions, 142 expected-error promotions). Promoted the 21 passing xfails from tck-gfql#142 into the direct-Cypher snapshot/contract, leaving tracked direct-Cypher non-validation debt at three wrong-row residuals and making no pygraphistry runtime changes.
- **Scenario Outline placeholder substitution audit (`<predicate>`, `<rhs>`, `<operands>`)**: Regenerated the 68 audited expression/quantifier/comparison xfail scenarios from the openCypher Scenario Outline examples so committed `scenario.cypher` strings no longer retain literal placeholders. Rebaselined the direct-Cypher xfail contract for the 13 scenarios that now execute past the old validation failure while remaining intentional expression-lane xfails: 11 `success_matches_expected` and 2 `success_wrong_rows` (tck-gfql#119).
- **Direct-Cypher promotion bookkeeping (`match5-8`, `with5-2`)**: Promoted the last `success_matches_expected` non-validation debt entries into the direct-Cypher row support snapshot after both keys passed the expected oracle locally, dropping tracked non-validation debt to zero and moving `with5-2` out of the row-pipeline xfail lane contract (tck-gfql#129).
- **Direct-Cypher node-identity branch drift (`match7-24`, `return-orderby2-6`, `expr-graph3-5`, counting subgraph use cases)**: Added the optional self-loop, count-star ORDER BY, graph-expression, and counting-subgraph scenarios to the sibling-target drift buckets after pygraphistry#1490 fixed internal node-identity handling for direct Cypher without conflating a Cypher `id` property with graph-bound node identity. The branch now tracks `match7-24`, `return-orderby2-6`, `expr-graph3-5`, and `usecase-countingsubgraphmatches1-{5,6,7,9}` as `success_matches_expected`, plus `usecase-countingsubgraphmatches1-2` as `success_wrong_rows`.
- **Graph fixture CREATE parser/model fix (`graph_fixture_from_create`)**: Resolved `<var>.<prop>` references against earlier nodes/bindings during fixture parsing and moved synthetic node identity to reserved `__node__`, preserving Cypher `id` as a real data property. Reverted `with2-1` to its upstream `graph_fixture_from_create` setup, added parser regressions for node property references and `id` preservation, and rebaselined the direct-Cypher snapshot to 926 promoted-only scenarios (787 rows, 139 errors). Seven prior direct-Cypher promotions returned to xfail because the corrected fixture exposes pygraphistry direct-Cypher assumptions that node identity lives in an `id` data column; `with5-2` is now tracked as `success_matches_expected` xfail drift but remains unpromoted by the row-pipeline tranche-2 contract (tck-gfql#126).
- **Direct-Cypher parameter-binding promotions (`$param` cluster)**: Recovered upstream TCK parameter values for the issue #120 cluster and promoted 13 row-supported direct-Cypher scenarios: `expr-list1-{3,4,5}`, `expr-list2-{10,11}`, `expr-map3-2`, `expr-null3-4-{1,2,3,5,6,7}`, and `expr-typeconversion2-6`. Rebaselined the direct-Cypher snapshot to 933 promoted-only scenarios (794 rows, 139 errors), added regression coverage for the audited parameter key set, and left remaining parameterized cases explicitly xfailed under their true residual gaps (unsupported writes/CREATE, percentile functions, and map dynamic access) (tck-gfql#120).
